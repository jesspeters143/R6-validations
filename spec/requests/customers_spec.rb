...
describe "get new_customer_path" do
  ...
  describe "put customer_path with valid data" do
    it "updates an entry and redirects to the show path for the customer" do
      customer = FactoryBot.create(:customer)
      put customer_path(customer.id), params: {customer: {first_name: "Jessica"} }
      customer.reload 
      expect(customer.first_name).to eq("Jessica")
      get customer_path
      expect(response).to render_template(:show)
    end
  end
  describe "put customer_path with invalid data" do
    it "does not update the customer record or redirect" do
      customer = FactoryBot.create(:customer)
      put customer_path(customer.id), params: {customer: {phone: "000"}}
      customer.reload
      expect(customer.phone).not_to eq("000")
      expect(response).to render_template(:edit)
    end
  end
